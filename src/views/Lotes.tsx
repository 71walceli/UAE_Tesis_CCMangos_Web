...
export const Lotes: React.FC = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      sort: true,
      filter: numberFilter(),
      formatter: p => ![undefined, null, ""].includes(p) ? formatNumber(getLength(parsePolygon(p))) 
        : null,
    },
  ];
  ...
  return (
    <RecordsScreen pageTitle="Lotes" columns={columns} controller={controller} 
      formManager={formManager} prepareSubmitForm={handleSubmit}
      readiness={[areasReady, variedadesReady, controller.ready]}
      formFields__React={<>
        <FormField
          name="Id_Lote"
          label="Área"
