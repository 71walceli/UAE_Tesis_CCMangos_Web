...
export const Lecturas: React.FC = () => {
  ...
  return (
    <RecordsScreen pageTitle="Controles de árbol" controller={controller} forbidCreate
      ...
      columns={[
        {
          ...
          formatter: v => `${findPlantaById(v).Codigo} : ${findPlantaById(v).NombreCompleto}`,
        },
        {
      ...
      formFields__React={<>
        {[
          {
            ...
            label: "Planta",
            inputType: "select",
            options: selectPlantaOptions(v => `${v.Codigo} : ${v.NombreCompleto}`),
          },
          {
            name: "FechaVisita",
            label: "Fecha",
        ...
        ].map((props, index) => <FormField key={index} {...props} />)}
      </>}
    />
  );
};
